...
#import "FBStickerView.h"

@interface FBARItemEffectView ()<UICollectionViewDataSource,UICollectionViewDelegate,QZImagePickerControllerDelegate,UIGestureRecognizerDelegate>
...
static NSString *const FBARItemEffectViewCellId = @"FBARItemEffectViewCellId";

@implementation FBARItemEffectView
...
- (UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        // 设置尾部间距
        //        layout.footerReferenceSize = CGSizeMake(0, FBHeight(50));
        ...
        _collectionView.alwaysBounceHorizontal = YES; // 默认是上下滑动，FB改成左右
        //        [_collectionView registerClass:[FBARItemEffectViewCell class] forCellWithReuseIdentifier:HTARItemEffectViewCellId];
...
- (instancetype)initWithFrame:(CGRect)frame listArr:(NSArray *)listArr{
    ...
    if (self) {
        ...
        self.arItmeType = FB_WaterMark; // FB单独拎出来展示水印
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(identifier == nil){
        ...
        [collectionView registerClass:[FBARItemEffectViewCell class] forCellWithReuseIdentifier:identifier];
    }
    
    FBARItemEffectViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexModel.download == 0){ // 未下载
        ...
        DownloadedType downloadedType = FB_DOWNLOAD_TYPE_None;
        NSString *arItemPath;
        NSString *jsonKey;
        
        switch (self.arItmeType) {
            case FB_Sticker:
                downloadedType = FB_DOWNLOAD_TYPE_Sticker;
                arItemPath = [[[FaceBeauty shareInstance] getARItemPathBy:FBItemSticker] stringByAppendingFormat:@"fb_sticker_config.json"];
                jsonKey = @"fb_sticker";
                break;
                
            case FB_Mask:
                downloadedType = FB_DOWNLOAD_TYPE_Mask;
                arItemPath = [[[FaceBeauty shareInstance] getARItemPathBy:FBItemMask] stringByAppendingFormat:@"fb_mask_config.json"];
                jsonKey = @"fb_mask";
                break;
        ...
        [[FBDownloadZipManager shareManager] downloadSuccessedType:downloadedType htModel:indexModel indexPath:indexPath completeBlock:^(BOOL successful, NSIndexPath *index) {
            ...
            if (successful) {
                ...
                [FBTool setWriteJsonDicFocKey:jsonKey index:index.row path:arItemPath];
            ...
            if (downloadedType == weakSelf.arItmeType+1) {
                // 如果下载完成后还在当前页，进行刷新
                [weakSelf.listArr replaceObjectAtIndex:index.row withObject:[FBTool getDictionaryWithFBModel:indexModel]];
        ...
        //互斥逻辑
        if(![FBTool mutualExclusion:FB_ARITEM_POSITION_MAP[self.arItmeType]]){
            return;
        }
...
- (void)generateEffectWithEffectType:(FBARItemType)effectType model:(FBModel *)model index:(NSInteger)index {
    
    switch (effectType) {
        case FB_Sticker:
            [FBTool setFloatValue:index forKey:FB_ARITEM_STICKER_POSITION];
            [[FaceBeauty shareInstance] setARItem:FBItemSticker name:model.name?:@""];
            break;
        case FB_Mask:
            [FBTool setFloatValue:index forKey:FB_ARITEM_MASK_POSITION];
            [[FaceBeauty shareInstance] setARItem:FBItemMask name:model.name?:@""];
            break;
        case FB_Gift:
            [FBTool setFloatValue:index forKey:FB_ARITEM_GIFT_POSITION];
            [[FaceBeauty shareInstance] setARItem:FBItemGift name:model.name?:@""];
            break;
        case FB_WaterMark:
            [FBTool setFloatValue:index forKey:FB_ARITEM_WATERMARK_POSITION];
            [[FaceBeauty shareInstance] setARItem:FBItemWatermark name:model.name?:@""];
            [self addWaterMarkView:model.name?:@""];
            //TODO:qzw setARItem:FBItemWatermark
            break;
...
- (void)updateDataWithDict:(NSDictionary *)dic{
    ...
    switch (self.arItmeType) {
        case FB_Sticker:
            selectedIndex = [FBTool getFloatValueForKey:FB_ARITEM_STICKER_POSITION];
            break;
        case FB_Mask:
            selectedIndex = [FBTool getFloatValueForKey:FB_ARITEM_MASK_POSITION];
            break;
        case FB_Gift:
            selectedIndex = [FBTool getFloatValueForKey:FB_ARITEM_GIFT_POSITION];
            break;
        case FB_WaterMark:
            selectedIndex = [FBTool getFloatValueForKey:FB_ARITEM_WATERMARK_POSITION];
            break;
    ...
    self.selectedModel = [[FBModel alloc] initWithDic:self.listArr[selectedIndex]];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithOriginImage:(UIImage *)image {
    ...
    NSString *filePath = [[FaceBeauty shareInstance] getARItemPathBy:FBItemWatermark];
    //    upload
...
//    UIImage *iconImage = [QZImagePickerController image:image scaleToSize:CGSizeMake(128, 128)];
    ...
    if (itmeImageResult == YES) {
        ...
        [FBTool addWriteJsonDicFocKey:@"watermark" newItme:newDic path:configPath]; // 原始@"fb_watermark"
...
-(void)deleteUploadItme:(NSInteger)index{
    ...
    NSMutableArray *configArray = [NSMutableArray arrayWithArray:[config objectForKey:@"watermark"]]; // 原始@"fb_watermark"
    ...
    [config setValue:configArray forKey:@"watermark"]; // 原始@"fb_watermark"
...
- (void)addWaterMarkView:(NSString *)name{
    ...
    for (UIView *old in [FBUIManager shareManager].superWindow.subviews) {
        ...
        //        if([old isKindOfClass:[FBStickerView class]]){
        //            [old removeFromSuperview];
        //        }
    }
    ...
    NSString *itmeFolder = [[[FaceBeauty shareInstance] getARItemPathBy:FBItemWatermark] stringByAppendingString:name];
    UIImage *im = [UIImage imageWithContentsOfFile:[NSString stringWithFormat:@"%@/%@.png",itmeFolder,name]];
    
    CGSize mainSize = [FBUIManager shareManager].superWindow.frame.size;
    CGSize resolutionSize = [FBUIManager shareManager].resolutionSize;
    CGRect bounds = CGRectMake(0, 0, im.size.width, im.size.height);
    CGRect imr = [FBStickerView superView:resolutionSize convertBounds:bounds toSize:mainSize];
    ...
    FBStickerView *stickerView = [[FBStickerView alloc] initWithContentView:imageView];
    stickerView.tag = 55555;
    stickerView.ctrlType = FBStickerViewCtrlTypeOne;
    ...
    [[FBUIManager shareManager].superWindow addSubview:stickerView];
    
//    [stickerView setUpdateCurrentLocationBlock:^(CGRect rect, float degrees, CGAffineTransform transform) {
//
//        CGSize mainSize = [[UIScreen mainScreen] bounds].size;
//        CGSize resolutionSize = [FBUIManager shareManager].resolutionSize;
//        CGRect r = [FBStickerView superView:mainSize convertBounds:rect toSize:resolutionSize];
//
//        //TODO:qzw setWatermarkParam
//        [[FaceBeauty shareInstance] setWatermarkParam:r.origin.x/resolutionSize.width y:r.origin.y/resolutionSize.height width:r.size.width height:r.size.height rotation:360-degrees];
//
//    }];
    ...
    [stickerView setUpdateCornerPositionBlock:^(CGPoint topLeft, CGPoint topRight, CGPoint bottomLeft, CGPoint bottomRight) {
        ...
        rect.origin = topLeft;
        CGRect rtopLeft = [FBStickerView superView:mainSize convertBounds:rect toSize:resolutionSize];
        rect.origin = topRight;
        CGRect rtopRight = [FBStickerView superView:mainSize convertBounds:rect toSize:resolutionSize];
        rect.origin = bottomLeft;
        CGRect rbottomLeft = [FBStickerView superView:mainSize convertBounds:rect toSize:resolutionSize];
        rect.origin = bottomRight;
        CGRect rbottomRight = [FBStickerView superView:mainSize convertBounds:rect toSize:resolutionSize];
        ...
        [[FaceBeauty shareInstance] setWatermarkParam:rtopLeft.origin.x / resolutionSize.width
