...
#import "FBBodyView.h"
#import "FBTool.h"
#import "FBBodyMenuView.h"
#import "FBUIConfig.h"
#import "FBUIManager.h"
#import "FBRestoreAlertView.h"
#import "FBBtBodyView.h"

@interface FBBodyView ()<FBRestoreAlertViewDelegate>
...
@property (nonatomic, strong) FBBodyMenuView *menuView;
@property (nonatomic, assign) BOOL menuViewDisabled;
@property (nonatomic, strong) UIView *lineView;
//@property (nonatomic, strong) UIButton *backButton;
//@property (nonatomic, strong) UIButton *cameraBtn;
@property (nonatomic, strong) FBBtBodyView *bodyView;
...
@implementation FBBodyView

- (instancetype)initWithFrame:(CGRect)frame{
    
    self = [super initWithFrame:frame];
    self.bodyArray = [FBTool jsonModeForPath:FBBodyBeautyPath withKey:@"FBBodyBeauty"];
    if (self) {
        self.currentType = FB_BODY_SLIDER;
...
- (void)setOnClickMenu:(NSArray *)array{
    ...
    if ([name isEqualToString:[FBTool isCurrentLanguageChinese] ? @"美体" : @"Body"]) {
        ...
        //默认选择第一个
        FBModel *model = [[FBModel alloc] initWithDic:self.bodyArray[0]];
...
- (void)updateEffect:(int)value{
    if (self.currentType == FB_BODY_SLIDER) {
        // 设置美体特效
...
- (void)alertViewDidSelectedStatus:(BOOL)status {
    ...
    if (status) {
        ...
        if (self.currentType == FB_BODY_SLIDER) {
            self.currentModel = [[FBModel alloc] initWithDic:self.bodyArray[0]];
...
- (NSArray *)listArr{
    if (!_listArr) {
        _listArr = @[
            @{
                @"name":[FBTool isCurrentLanguageChinese] ? @"美体" : @"Body",
                @"classify":@[
                    @{
                        @"name":[FBTool isCurrentLanguageChinese] ? @"美体" : @"Body",
...
- (FBBodyMenuView *)menuView{
    if (!_menuView) {
        _menuView = [[FBBodyMenuView alloc] initWithFrame:CGRectZero listArr:self.listArr];
        WeakSelf
...
- (FBBtBodyView *)bodyView{
    if (!_bodyView) {
        _bodyView = [[FBBtBodyView alloc] initWithFrame:CGRectZero listArr:self.bodyArray];
        WeakSelf
        // 展示弹框
        ...
        // 通知滑动条
        _bodyView.bodyDidSelectedBlock = ^(FBModel * _Nonnull model) {
