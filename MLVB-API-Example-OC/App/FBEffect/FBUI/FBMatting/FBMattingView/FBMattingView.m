...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        aiSegmentationPath = [[[FaceBeauty shareInstance] getAISegEffectPath] stringByAppendingFormat:@"fb_aiseg_effect_config.json"];
        greenscreenPath = [[[FaceBeauty shareInstance] getChromaKeyingPath] stringByAppendingFormat:@"fb_gsseg_effect_config.json"];
...
- (NSArray *)listArr{
    _listArr = @[
        @{
            ...
            @"classify":[FBTool jsonModeForPath:aiSegmentationPath withKey:@"fb_aiseg_effect"]
        },
        @{
            @"name":[FBTool isCurrentLanguageChinese] ? @"绿幕抠图" : @"Chroma Keying",
            @"classify":[FBTool jsonModeForPath:greenscreenPath withKey:@"fb_gsseg_effect"]
        }
    ];
    return _listArr;
}
...
- (FBMattingMenuView *)menuView{
    if (!_menuView) {
        _menuView = [[FBMattingMenuView alloc] initWithFrame:CGRectZero listArr:self.listArr];
        WeakSelf
